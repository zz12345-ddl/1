<template>
    <div class="homeBox" :class="{ isWeb: !isTauri }">
        ...
        >
            <template #header>
                <div class="diaHeader">
                    <span>{{ t('modeSelection') }}</span>
                </div>
            </template>
            <div class="modeContent">
                <div class="modeOption" @click="selectMode('local')">
                    <div class="modeIcon">
                        <span class="iconfont">&#xe6a4;</span>
                    </div>
                    <div class="modeInfo">
                        <div class="modeTitle">{{ t('localMode') }}</div>
                        <div class="modeDesc">{{ t('localModeDesc') }}</div>
                    </div>
                </div>
                <div class="modeOption" @click="selectMode('cloud')">
                    <div class="modeIcon">
                        <span class="iconfont">&#xe709;</span>
                    </div>
                    <div class="modeInfo">
                        <div class="modeTitle">{{ t('cloudMode') }}</div>
                        <div class="modeDesc">{{ t('cloudModeDesc') }}</div>
                    </div>
                </div>
            </div>
            <template #footer>
                <div class="dialog-footer">
                    <el-button @click="modeDialog = false">
                        {{ t('cancel') }}
                    </el-button>
                </div>
            </template>
        ...
        >
            <template #header>
                <div class="diaHeader">
                    ...
                    <span v-if="usageMode === 'local'" class="modeTag localTag">{{ t('localMode') }}</span>
                    <span v-else class="modeTag cloudTag">{{ t('cloudMode') }}</span>
                </div>
            </template>
...
// usage mode: 'local' or 'cloud'
const usageMode = ref('local')
...
const creatProject = async () => {
    ...
    if (branchName.value && /^[A-Za-z][A-Za-z0-9]*$/.test(branchName.value)) {
        ...
        // check branch exist
        if (usageMode.value === 'cloud' && store.token) {
            ...
            const projectConfig = {
                ...
                appid: `com.${branchName.value}.app`,
                customJs: customJs,
                usageMode: usageMode.value, // 添加使用模式标识
                more: {
                    ...ppconfig.more,
                    windows: {
                        ...ppconfig.more.windows,
            ...
            store.setCurrentProject(projectConfig)
            creatLoading.value = false
            branchDialog.value = false
            router.push('/edit')
        }
