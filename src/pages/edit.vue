...
const publishWeb = async () => {
    // 检查使用模式
    const isCloudMode = store.currentProject.usageMode === 'cloud'
    
    if (isCloudMode && store.token === '') {
        oneMessage.warning(t('noTokenLocalBuild'))
        // 继续执行本地构建流程，不阻止用户操作
    }
    
    if (isCloudMode && checkLastPublish()) {
    ...
    try {
        if (isCloudMode) {
            ...
            // 本地模式下的构建完成提示
            oneMessage.success(t('localBuildComplete'))
            buildLoading.value = false
        }
    } catch (error: any) {
        warning.value = error.message
