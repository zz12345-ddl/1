export default {
  ...
  // 使用模式相关
  noTokenWarning: '토큰이 설정되지 않았습니다. 로컬 모드로 프로젝트를 생성하는 것을 권장합니다.',
  localBuildTip: '로컬 모드에서는 프로젝트가 로컬에서만 빌드되며 Github에 업로드되지 않습니다.',
  usageMode: '사용 모드',
  localMode: '로컬 모드',
  cloudMode: '클라우드 모드',
  localModeDesc: '프로젝트를 로컬에서 빌드하고 Github에 업로드하지 않습니다',
  cloudModeDesc: '프로젝트를 Github에 업로드하고 클라우드에서 빌드합니다',
  selectUsageMode: '사용 모드를 선택하세요',
  modeSelection: '모드 선택',
  localBuildStart: '로컬 빌드 시작',
  localBuildComplete: '로컬 빌드 완료'
}
